...
CREATE INDEX idx_guides_status ON guides(processing_status);
CREATE INDEX idx_guides_source_url ON guides(source_url);
...
CREATE INDEX idx_guide_content_search ON guide_content USING GIN(search_vector);
CREATE INDEX idx_guide_content_guide_id ON guide_content(guide_id);
...
CREATE INDEX idx_chunks_embedding_hnsw ON guide_chunks
USING hnsw (embedding vector_cosine_ops)
WITH (m = 16, ef_construction = 64);

CREATE INDEX idx_chunks_guide_id ON guide_chunks(guide_id);
...
CREATE INDEX idx_conversations_guide_id ON conversations(guide_id);
...
CREATE INDEX idx_cached_content_url_hash ON cached_content(url_hash);
CREATE INDEX idx_cached_content_expires_at ON cached_content(expires_at);
...
CREATE INDEX idx_api_usage_user_id_date ON api_usage(user_id, date DESC);
